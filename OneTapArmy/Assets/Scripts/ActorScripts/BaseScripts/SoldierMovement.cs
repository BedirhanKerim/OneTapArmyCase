...
namespace OneTapArmyCore
{
    public class SoldierMovement : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            switch (teamType)
            {
                case TeamType.PlayerBlue:
                    defaultLookDirection = Quaternion.LookRotation(Vector3.forward);
                    break;
                case TeamType.EnemyRed:
                    defaultLookDirection = Quaternion.LookRotation(Vector3.back);
                    break;
                case TeamType.EnemyYellow:
                    defaultLookDirection = Quaternion.LookRotation(Vector3.right);
                    break;
            }
        }
        ...
        public void Move()
        {
            ...
            if (movementIsDone)
            {
                targetRotation = defaultLookDirection;
        ...
        private void HandleMovement(Vector3 moveDirection, Quaternion targetRotation)
        {
            ...
            if (distance < 0.001f || ((targetLocation.x < 4 || targetLocation.x > 13) && distance < 3f))
            {
                movementIsDone = true;
