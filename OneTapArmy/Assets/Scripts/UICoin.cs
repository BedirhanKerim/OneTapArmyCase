...
public class UICoin : MonoBehaviour
{
   private Tween _moveTween;
    ...
    public void Move(Vector3 origin, Vector3 currentPos, Vector3 targetPos, float duration, Action callback, int coinAmount)
    {
        _moveTween?.Kill();
        _moveTweenX?.Kill();
        _moveTweenY?.Kill();
        ...
        _moveTween = transform.DOMove(currentPos, duration)
            .OnComplete(()=> 
            {
                _ = MoveToTheEndPoint(currentPos, targetPos, deltaX, callback, coinAmount);
            });
    }

    
    private async UniTaskVoid MoveToTheEndPoint(Vector3 startPos, Vector3 endPos, float height, Action callback, int coinAmount)
    {
        ...
        try
        {
            while (transform != null)
            {
                ...
                else
                {
                    ...
                    GameManager.Instance.AddGold(coinAmount);
